...
export default function EditDialog({ uploadClose, uploadOpen, row, tableData }) {
  ...
  const NewRoomSchema = Yup.object().shape({
    Dpt_Name: Yup.string()
      ...
      .max(100, 'Room Name must be less than or equal to 100 characters'),
    // .matches(/^[a-zA-Z\s]+$/, 'Room Name must only contain letters and spaces'),
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (tableData.some((item) => item.Dpt_Name === data.Dpt_Name)) {
      enqueueSnackbar('Room Name already exists', { variant: 'error' });
      return;
    }
    try {
      const dataToSend = {
        ...
        <DialogContent sx={{ py: 3 }}>
          <FormProvider methods={methods} onSubmit={onSubmit}>
            ...
            >
              ...
              />
              <RHFTextField name="Dpt_Name" label="Room Name" />
            </Box>
